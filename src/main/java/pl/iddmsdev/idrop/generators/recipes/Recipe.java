...
//        setupCoreSlots();
    ...
    private void setIngredients() {
        for(String str : recipes.getStringList("recipes."+recipeIdentifier+".ingredients")) {
            ...
            if(spl[1].equals("core")) {
                int coreLevel = recipes.getInt("recipes." + recipeIdentifier + ".required-core-lvl");
                this.setIngredient(spl[0].toUpperCase().charAt(0), new RecipeChoice.ExactChoice(GeneratorCore.getCore(coreLevel)));
            } else {
    ...
    private void addCraftingSlot() {
        ...
        for(int i = 0; i<9; i++) {
            crafting.add(getIngredientMap().get(indexes[i]));
        }
    }
